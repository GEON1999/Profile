    ...
    work: [
      {
        ...
        description: [
          "PRD·도메인 용어집·ADR 10건을 먼저 세운 뒤 이슈 기반 PR 워크플로로 MVP 완성. 모든 기술 결정의 근거를 ADR로 문서화",
        ],
      },
      {
        title: "지원 파이프라인 칸반",
        description: [
          "dnd-kit 드래그앤드롭으로 단계 이동, 단계 전환 로그를 근거로 현재 단계 체류 일수와 팔로업 필요 배지를 자동 계산",
        ],
      },
      {
        title: "공고 스냅샷 + 제출 문서 라이브러리",
        description: [
          "마감되면 사라지는 공고 원문을 지원 시점 그대로 보존, 이력서 버전과 지원 건을 N:M으로 매핑해 양방향 조회",
        ],
      },
      {
        title: "면접 회고 + 질문 은행",
        description: [
          "면접에서 받은 질문을 태그·검색 가능한 형태로 축적해, 휘발되던 면접 경험을 다음 면접 대비 자산으로 전환",
        ],
      },
      {
        title: "전환율 대시보드 + ICS 캘린더 피드",
        description: [
          "Recharts로 단계별 전환율·체류 일수를 시각화, 다음 액션을 HMAC 토큰 인증 ICS 피드로 캘린더에 구독",
        ],
      },
      {
        title: "개인 도구 → 공개 가입 오픈 서비스 전환",
        description: [
          "앱 레벨 멀티테넌시로 유저별 데이터 격리, Turnstile 캡차와 스토리지 총량 쿼터로 남용·비용 방어",
        ],
      },
    ],
    issues: [
      {
        ...
        description: [
          "탈락을 하나의 진행 단계(칸반 컬럼)로 모델링하면 구현은 단순하지만, 어느 단계에서 종료됐는지가 사라져 병목 분석 자체가 성립하지 않음",
          "시간이 지날수록 탈락 컬럼만 비대해져 보드의 정보 가치도 함께 떨어짐",
        ],
        techDetails: [
          {
            ...
            reasoning: [
              ...
              "이벤트 소싱은 모든 전환을 이벤트로 남겨 같은 문제를 풀 수 있으나, 단일 유저 CRUD 앱에는 과잉이라 단순 전환 로그 테이블로 한정",
            ],
          },
        ],
        solution: [
          "진행 단계(지원함→서류→과제→면접→오퍼)와 결과(진행중/탈락/철회/수락)를 직교 조합으로 모델링 — '서류에서 탈락'은 Stage=서류 × Outcome=탈락으로 보존",
          "종료된 지원은 마지막 단계 정보를 유지한 채 칸반에서 아카이브로 분리",
          "체류 일수 계산 근거로 단계 전환 이력 테이블을 별도로 분리",
        ],
        result: [
          "'어느 단계에서 떨어졌는지'가 데이터로 남아 단계별 전환율 대시보드가 성립",
          "전환율 분모를 결판난 건(넘어간 건 + 그 단계에서 종료된 건)으로 한정해, 대기 중인 지원이 통과율을 체계적으로 낮게 왜곡하는 문제 제거",
        ],
      },
      {
        title: "공고 원문 수집 — 크롤링 실패율 vs 아카이브의 저장 신뢰성",
        description: [
          "채용 플랫폼은 로그인월·봇 차단·SPA 동적 렌더링이 겹쳐 크롤링 실패율이 높고, 사이트별 파서 유지보수는 프로젝트 범위를 넘어섬",
          "아카이브 도구에서 최악의 결과는 UX 불편이 아니라 '저장했다고 믿었는데 비어 있는' 상태",
        ],
        techDetails: [
          {
            category: "저장 신뢰성을 편의성보다 우선",
            reasoning: [
              "URL 자동 크롤링은 UX가 좋지만 실패가 조용히 누적되면 아카이브의 존재 이유 자체가 무너짐",
              "수동 붙여넣기는 UX 비용을 지불하는 대신 저장 성공이 보장되고, 사용자가 저장 내용을 눈으로 확인한 상태로 남음",
            ],
          },
        ],
        solution: [
          "공고 본문을 수동 붙여넣기로 수집하고 원본 URL·저장 시각을 메타로 함께 기록",
          "수정은 허용하되 '지원 시점 원문 보존'이 목적임을 UI에 명시, 지원당 스냅샷 1건으로 한정해 재지원 시 새 스냅샷을 남김",
        ],
        result: [
          "공고가 내려가도 지원 시점 원문을 다시 확인할 수 있는 저장 경로 확보",
          "크롤링은 수동 입력의 대체가 아니라 프리필 보조로 범위를 못 박아 향후 과제로 분리",
        ],
      },
      {
        title: "개인 도구를 공개 가입 오픈 서비스로 전환",
        description: [
          "채용 담당자가 직접 가입해 체험할 수 있게 공개 전환이 필요했으나, '회원가입 없음'이 미구현이 아니라 의도된 설계였던 탓에 모든 테이블에 소유자 컬럼이 없고 쿼리에 유저 필터가 전무한 상태",
          "ORM이 DB에 직접 연결해 RLS를 우회하는 구조라, 전환의 핵심은 가입 폼이 아니라 데이터 소유권 모델 도입이었음",
        ],
        techDetails: [
          {
            category: "앱 레벨 필터링 vs DB 레벨 RLS",
            reasoning: [
              "DB 레벨 RLS가 심층 방어로는 우월하나, 트랜잭션마다 유저 클레임을 싣도록 데이터 접근 구조를 재설계해야 해 MVP 규모 대비 비용이 과도",
              "대신 userId를 쿼리 함수의 명시적 인자로 올려 함수 시그니처만 봐도 격리 여부가 드러나게 하고, RLS 하드닝은 향후 과제로 명시",
            ],
          },
          {
            category: "소유자 컬럼을 어느 테이블까지 둘 것인가",
            reasoning: [
              "전 테이블 denormalize는 단독 필터가 가능하지만, 앱 레벨 격리에서는 부모-자식 불일치가 곧 데이터 유출",
              "루트 테이블에만 두고 자식은 부모 조인으로 소유권을 파생시켜 정합성 리스크와 백필 범위를 동시에 축소",
            ],
          },
        ],
        solution: [
          "루트 3개 테이블(회사·지원·문서)에만 userId를 두고 자식 엔티티는 부모 조인으로 소유권 검증, 기존 데이터는 오너 계정으로 백필",
          "스토리지 경로를 유저 폴더로 네임스페이싱하고 다운로드 서명 전 소유권을 반드시 재검증",
          "가입 마찰을 없애기 위해 이메일 인증 대신 Turnstile 캡차 + 스토리지 총량 쿼터로 봇·비용 남용을 방어",
        ],
        result: [
          "누구나 가입해 자기 데이터만 격리된 상태로 체험 가능한 오픈 서비스로 전환, 기존 개인 데이터는 무중단 유지",
          "회사명 전역 유니크를 (유저, 회사명) 복합 유니크로 바꿔 유저 간 이름 충돌 제거",
        ],
      },
    ],
